...
[System.Serializable]
[Node(false, "Conditional Logic/Or Node")]
public class OrNode : ConditionNode
{
    ...
    public override Node Create(Vector2 pos)
    {
        ...
        node.ID = "Or Node " + node.GetInstanceID();
    ...
    public override void NodeGUI()
    {
        ...
        if (Inputs[0].connections.Count > 0 && Inputs[0].connections[0].body != null)
            GUILayout.Label((Inputs[0].connections[0].body as ConditionNode).Check().ToString());
        else
            try
            {
#if UNITY_EDITOR
        ...
        if (Inputs[1].connections.Count > 0 && Inputs[1].connections[0].body != null)
            GUILayout.Label((Inputs[1].connections[0].body as ConditionNode).Check().ToString());
        else
            try
            {
#if UNITY_EDITOR
    ...
    /// <summary>
    /// Weither either condition a or b is true
    /// </summary>
    /// <returns>If either is true</returns>
